...
/* Boo Attacked Type Not Needed as Boo's are Now Invincible 
...
static void boo_chase_mario(f32 minDY, s16 yawIncrement, f32 mul) {
    ...
    if (boo_vanish_or_appear()) {
        ...
        cur_obj_rotate_yaw_toward(targetYaw, yawIncrement * 2.5f);
        o->oVelY = 0.0f;

        if (!mario_is_in_air_action()) {
            f32 dy = o->oPosY - gMarioObject->oPosY;
            if ((minDY < dy) && (dy < 500.0f)) {
                o->oVelY = increment_velocity_toward_range(o->oPosY, gMarioObject->oPosY + 50.0f, 10.0f, 10.0f);
            }
        }
